...
struct Segment {
    vaddr_t vaddr{0};                   // where in the address space
...
struct ElfImage {
    ...
    vaddr_t entry_point{0};         // e_entry in ELF header -- the address execution should start at
    ...
    std::unordered_map<std::string, vaddr_t> symbols_map;
    ...
    [[nodiscard]] vaddr_t highest_extent() const {
        vaddr_t highest = 0;
